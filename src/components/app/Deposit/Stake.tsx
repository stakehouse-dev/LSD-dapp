...
export const Stake: FC<StakeProps> = ({ onBack }) => {
  const [amount, setAmount] = useState<string>('')
  const [txResult, setTxResult] = useState<any>()
  const [failed, setFailed] = useState<string>('')

  const { handleDeposit, isLoading, setLoading } = useDepositProtectedStaking()
  ...
  const handleDepositEth = async () => {
    try {
      ...
      console.log('deposit protected staking error-----------------')
