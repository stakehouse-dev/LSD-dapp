...
export const useFacilitatorRageQuit = (blsKey?: string) => {
  const { sdk } = useSDK()
  ...
  const [extraStep, setExtraStep] = useState<boolean>(false)
  ...
  const handleTopUpDETH = useCallback(
    async (_requiredDETH: string) => {
      if (sdk && _requiredDETH) {
        const requiredDETHWei = parseEther(_requiredDETH)
        ...
        try {
          ...
          const allowanceTx = await dETH.approve(rageQuitAssistantAddress, requiredDETHWei)
  ...
  const handleWithdrawValidator = useCallback(async () => {
    if (sdk && lsdvalidator && blsKey && stakehouseAccount && account) {
      setLoading(true)
  ...
  }, [sdk, lsdvalidator, blsKey, stakehouseAccount, account])
